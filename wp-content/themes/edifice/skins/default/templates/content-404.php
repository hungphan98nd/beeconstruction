<article <?php post_class( 'post_item_single post_item_404' ); ?>>
	<div class="post_content">
		...
		<div class="page_info">
			...
			<a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="go_home theme_button sc_button sc_button_decoration"><?php esc_html_e( 'Homepage', 'edifice' ); ?></a>
		</div>

		<?php
		// SVG
		$svg_bg_2 = edifice_get_svg_from_file(edifice_get_file_dir('images/svg_bg_2.svg'));
		...
		$svg_bg = ($svg_bg_2 ? '<span class="svg-2">'.$svg_bg_2.'</span>' : '').($svg_bg_3 ? '<span class="svg-3">'.$svg_bg_3.'</span>' : '').($svg_bg_4 ? '<span class="svg-4">'.$svg_bg_4.'</span>' : '');
		if(!empty($svg_bg)){ ?>
		<div class="all-svg">
